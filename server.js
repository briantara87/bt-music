...
client.on("ready", () => {
  console.log(
    `${client.user.tag} ready to serving ${client.guilds.cache.size} guild(s) and ${client.users.cache.size} user(s), with ${client.commands.size} command(s) total!`
  );
...
You can type ${prefix}help to get bot commands list`)
  ...
  try {
    ...
    if(!file) return msg.reply("nothing")
